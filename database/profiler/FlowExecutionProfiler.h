...
#define INIT_FLOW_EXECUTION_TIME_PROFILER \
	...
	for(size_t i = 0; i < gThreadCount; ++i) { slice_busy_time_stat_[i] = new long long[kMaxSliceNum]; memset(slice_busy_time_stat_[i], 0, kMaxSliceNum * sizeof(long long)); } \
	slice_wait_time_stat_ = new long long*[gThreadCount]; \
	for(size_t i = 0; i < gThreadCount; ++i) { slice_wait_time_stat_[i] = new long long[kMaxSliceNum]; memset(slice_wait_time_stat_[i], 0, kMaxSliceNum * sizeof(long long)); } \
